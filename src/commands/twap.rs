...
fn state_path(symbol: &str, exchange: &str) -> PathBuf {
    ...
    PathBuf::from(home).join(format!(".twap-{}-{}.json", symbol.to_lowercase(), exchange.to_lowercase()))
}
...
pub async fn execute(args: TwapArgs, settings: &AppConfig) -> Result<()> {
    ...
    let (slices, interval_secs, slice_usd, mut total_spent, mut total_qty, mut filled_slices, completed_indices) =
        if args.resume {
            match load_state(&args.symbol, &args.exchange) {
                Some(state) => {
                    ...
                    println!();
                    println!("  RESUMING TWAP — {} {} on {}", state.symbol, state.side.to_uppercase(), state.exchange);
                    println!("  Progress:  {}/{} slices already filled  (${:.2} deployed)", state.filled_slices, state.slices, state.total_spent);
                    println!("  Remaining: {} slices × ${:.2} each", state.slices - state.filled_slices, state.slice_usd);
                    println!("  ─────────────────────────────────────────────────────");
                    println!();
                    (state.slices, state.interval_secs, state.slice_usd, state.total_spent, state.total_qty, state.filled_slices, state.completed_indices)
                }
                None => {
                    return Err(TtcError::InvalidOrder(format!(
            ...
            let slices = args.slices.unwrap_or_else(|| {
                (args.budget / min_usd).floor() as u32
            }).max(1);
            ...
            println!("  Budget:   ${:.2} notional over {:.1}h", args.budget, args.hours);
            println!("  Slices:   {} orders × ${:.2} each  (min entry: ${:.2})", slices, slice_usd, min_usd);
            println!("  Interval: {}m {}s between orders", interval_secs / 60, interval_secs % 60);
            if let Some(lev) = args.leverage {
                println!("  Leverage: {}x  (margin required: ${:.2})", lev, args.budget / lev as f64);
            }
            println!("  State:    ~/.twap-{}-{}.json", args.symbol.to_lowercase(), args.exchange.to_lowercase());
            println!("  ─────────────────────────────────────────────────────");
            println!();
            ...
            if settings.trading.dry_run {
                ...
                if let Some(lev) = args.leverage {
                    println!("  DRY-RUN: Margin required: ${:.2}  (${:.2} / {}x)", args.budget / lev as f64, args.budget, lev);
                }
                return Ok(());
            }
            ...
            if let Some(lev) = args.leverage {
                let lev_params = SetLeverageParams { symbol: args.symbol.clone(), leverage: lev };
                match client.set_leverage(&args.exchange, lev_params, credentials.clone()).await {
                    Ok(_) => println!("  Leverage set to {}x on {}", lev, args.exchange),
                    Err(e) => println!("  WARNING: Could not set leverage: {} — continuing anyway", e),
                }
                println!();
            }
            ...
            (slices, interval_secs, slice_usd, 0.0_f64, 0.0_f64, 0u32, vec![])
        };
    ...
    for i in 1..=slices {
        ...
        if completed_indices.contains(&i) {
            println!("  [{}/{}]  SKIPPED (already filled in previous run)", i, slices);
            continue;
        }
    ...
    if filled_slices == slices {
        ...
        println!("  {} slices were skipped or errored.", slices - filled_slices);
