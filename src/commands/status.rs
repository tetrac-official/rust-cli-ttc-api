...
fn check_session(settings: &AppConfig) -> (bool, String) {
    let token_set = settings.api_key.as_ref().map(|k| !k.is_empty()).unwrap_or(false);
...
fn check_credentials(settings: &AppConfig) -> (bool, String) {
    ...
    let known = ["ORDERLY", "BYBIT", "OKX", "BINANCE", "BITGET", "BLOFIN", "KUCOIN"];
    for prefix in &known {
